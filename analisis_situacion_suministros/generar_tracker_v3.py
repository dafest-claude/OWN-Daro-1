...
PLAN_FILE     = find_plan('180926')          # corte actual
PLAN_FILE_OLD = find_plan('110926')          # corte anterior (para diff de críticos)
# Cortes para la evolución (rolling window de 5) — se muestran como tendencia
PLAN_TREND    = [
    ('21/08', find_plan('210826')),
    ('28/08', find_plan('280826')),
    ('04/09', find_plan('040926')),
    ('11/09', find_plan('110926')),
    ('18/09', find_plan('180926')),
]
OUT_XLSX      = os.path.join(SCRIPT_DIR, 'Tracker_Suministros_IN_EL_LaCalera_II_Rev15_180926.xlsx')
TODAY         = date(2026, 9, 18)
RFSU          = date(2027, 2, 3)
VERSION       = 'Rev15_180926'
...
def build_cambios_semana(wb, sc_old, sc_new, trend=None):
    ...
    t = ws['A1']
    t.value = 'EVOLUCIÓN: Plan 21/08 → 28/08 → 04/09 → 11/09 → 18/09/2026 | CAMBIOS Y AVANCES'
    ...
    s2.value = ('Especialidades: IN (Instrumentación & Control)  |  EL (Electricidad)  |  '
                'Proyecto: La Calera II CPF2  |  Corte actual: 18/09/2026 (Rev15)')
    ...
    sub = ws.cell(base, 1, 'DETALLE DE CAMBIOS POR ÍTEM – SUMINISTROS CRÍTICOS IN / EL (11/09 → 18/09)')
    ...
    r = base
    hdrs = ['N°', 'Esp', 'Suministro / Ítem', 'Campo', 'Plan 110926 (anterior)', 'Plan 180926 (actual)', 'Tipo de cambio']
    for ci, h in enumerate(hdrs, start=1):
        c = ws.cell(r, ci, h)
...
def build_portada(wb):
    ...
    data = [
        ...
        ('Fuente plan',        '14.09.2026 – Plan de Suministros – La Calera II (180926).xlsx · Rev15'),
...
def build_cambios(wb):
    ...
    cambios = [
        ...
        ('28', 'REVISIÓN 15 (180926)',
         'Rev15: Plan 180926 (18/09/2026). Comparación 110926 → 180926. '
         '+4 OCs IN+EL (32 → 36): Instrumentación 22 → 25, Electricidad 10 → 11 (items bulk). '
         'Petición de ofertas casi vaciada (IN 3 → 0, EL 3 → 1). AT estable. KPIs: 69 RIs, 66 en gestión, 3 sin emitir. '
         'SE#3 (ABB): se envían PL de trafos, ABB devuelve comentarios con posibles modificaciones por CAT – proveedor evalúa impacto en PRECIO y PLAZO (a seguir). '
         'SIS (HIMA): en ida y vuelta de consultas (PP/AESA) para recotizar los adicionales. '
         'VÁLVULAS (único crítico sin OC, 148 d): sigue pendiente la validación final de ofertas para adjudicar (flete aéreo). Cables ya con OC + 2da instancia.'),
        ('27', 'REVISIÓN 14 (110926)',
         'Rev14: Plan 110926 (11/09/2026). Comparación 040926 → 110926. '
         'NUEVA SEMANA FUERTE DE OCs: Instrumentación 16 → 22 (+6 OCs, items bulk) y Electricidad 9 → 10 (+1); '
         'total OCs IN+EL 25 → 32. AT IN 29 → 23 (los items avanzaron a OC). KPIs: 69 RIs, 66 en gestión, 3 sin emitir. '
         'CABLES – 2da instancia: Instrumentación suma FO por comprar (3,5 km Rev.0 + 31 km Rev.1); Eléctricos 5 km Rev.1. '
         'VÁLVULAS (único crítico sin OC, 141 d): sigue pendiente la validación final de ofertas para adjudicar (flete aéreo). '
         'SIS: posible aumento de señales, licitación por adicionales en curso.'),
        ('26', 'REVISIÓN 13 (040926)',
         'Rev13: Plan 040926 (04/09/2026). Comparación 280826 → 040926. '
         'Semana de consolidación: el Cuadro resumen no cambia (KPIs 69/25/66/3). '
         'CABLES – 2da instancia en marcha: Instrumentación licita 29.000 m (SOLPED 23493830 liberada); '
         'Eléctricos licita un parcial menor (SOLPED 23501690 liberada). Ambos ya con OC de la 1ra instancia. '
         'VÁLVULAS (único crítico sin OC, 134 d): pendiente la VALIDACIÓN FINAL de ofertas para adjudicar; se avanza con flete aéreo. '
         'SIS (HIMA): se analizan dimensiones de gabinetes – habría MÁS SEÑALES que las previstas; se lanza licitación por adicionales (servicio). '
         'SE#3 (ABB): ductos de barras en 2da instancia (pendiente confirmar info de transformadores).'),
...
# ── MAIN ────────────────────────────────────────────────────────────────────
def main():
    print(f'[1/7] Leyendo Plan 180926 (actual, Rev15): {os.path.basename(PLAN_FILE)}')
    ...
    print(f'[2/7] Leyendo Plan 110926 (corte anterior, para comparación): {os.path.basename(PLAN_FILE_OLD)}')
    ...
    print('[4/7] Generando hoja CAMBIOS SEMANA (evolución 110926 → 180926)...')
