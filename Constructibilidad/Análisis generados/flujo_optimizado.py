...
# -*- coding: utf-8 -*-
"""
Diagrama de flujo VERTICAL time-scaled del CRONOGRAMA OPTIMIZADO - CPF2 La Calera II
Escenario con palancas: cables progresivos (inicio c/20%), 6 d/sem, doble turno en
conexionado de salas y Sala INS, solapamiento parcial precom-comisionado.
Carriles: SUMINISTROS · INSTRUMENTACIÓN · ELECTRICIDAD · CONFLUENCIA. Ruta crítica en rojo.
RFSU: base 31-ENE-28 · optimizado 31-MAR-28 · conservador 31-MAY-28.
Salida: Flujo_Optimizado_CPF2.pdf
"""
import os, math
...
OUT=os.path.join(BASE,"Flujo_Optimizado_CPF2.pdf")
W,H=A4
...
T0=D('2027-01-01'); T1=D('2028-06-30'); ytop=H-26*mm; ybot=22*mm
...
c.drawCentredString(W/2,H-9.5*mm,"Flujo OPTIMIZADO de Ejecución — EL + IN")
c.setFont('Helvetica',8); c.setFillColor(colors.HexColor('#9db8d8'))
c.drawCentredString(W/2,H-14.5*mm,"CPF2 La Calera II · cables progresivos · 6 d/sem · doble turno en salas/Sala INS · solapamiento precom-comis · RFSU optim. 31-MAR-28")
axx=22*mm
...
for dt,col,lbl in [(D('2028-01-31'),GREY,'RFSU base'),(D('2028-03-31'),RED,'RFSU optimizado'),(D('2028-05-31'),colors.HexColor('#bbbbbb'),'RFSU conservador')]:
...
# SUMINISTROS
ms(38*mm,'2027-01-20','Cables (progresivos)',GOLD,False)
for a,lbl,cr in [('2027-04-18','SIS',False),('2027-05-24','SE#3',False),('2027-06-02','PMS',False),
 ('2027-06-23','SE#4',False),('2027-06-24','PCS (Inauco)',True),('2027-07-20','Skids comb.',True)]:
    ms(38*mm,a,lbl,GOLD,cr)
# IN
bar(64*mm,'2027-01-01','2027-03-31','Canalizaciones IN',IN)
bar(64*mm,'2027-06-01','2027-11-15','Montaje instrumentos',IN)
bar(76*mm,'2027-02-15','2027-07-15','Tendido IN (c/20%·6d)',IN,crit=True)
bar(76*mm,'2027-07-01','2027-11-15','Conexionado IN (2 turnos)',IN,crit=True)
bar(64*mm,'2027-10-15','2028-01-31','Loop check IN',IN,crit=True)
# EL
bar(98*mm,'2027-02-01','2027-04-15','Bandejas EL',EL)
bar(98*mm,'2027-05-24','2027-07-15','Armado salas SE#3/4',EL)
bar(110*mm,'2027-03-01','2027-07-31','Tendido EL (c/20%·6d)',EL,crit=True)
bar(98*mm,'2027-07-01','2027-10-15','Montaje tableros/motores',EL)
bar(110*mm,'2027-07-15','2027-10-31','Conexionado EL (2 turnos)',EL,crit=True)
bar(110*mm,'2027-10-15','2027-11-15','Megger',EL,crit=True)
# CONFLUENCIA
bar(140*mm,'2027-11-16','2027-12-15','Energización',GREEN,crit=True)
bar(140*mm,'2027-12-01','2028-02-15','Precomisionado',GREEN,crit=True)
bar(154*mm,'2028-01-15','2028-03-31','Comisionado (solapa parc.)',PURP,crit=True)
ms(154*mm,'2028-01-31','RFSU base',GREY,False)
ms(168*mm,'2028-03-31','★ RFSU optimizado',RED,True)
# flechas
c.setDash(2,2)
...
carr(76*mm,'2027-11-15',140*mm,'2027-12-01',IN)
carr(110*mm,'2027-10-31',140*mm,'2027-11-20',EL)
carr(140*mm,'2028-02-15',154*mm,'2028-02-10',GREEN)
c.setDash()
...
c.drawString(axx,9.5*mm,"Palancas: cables progresivos · 6 d/sem · doble turno en conexionado de salas y Sala INS · solapamiento parcial precom-comisionado.")
c.drawString(axx,6*mm,"RFSU optimizado ≈ 31-MAR-28 (recupera ~2 meses vs conservador 31-MAY). Piso duro: PCS 24-jun y skids 20-jul-27. Preliminar para análisis.")
